...
export default function CatSprite({ speech, direction = 1, isColliding = false }) {
  // Flip sprite based on direction (1 for right, -1 for left)
  ...
  return (
    <div className="relative">
      {/* Collision indicator (only shows when colliding) */}
      {isColliding && (
        <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 
          bg-red-500 text-white text-xs px-2 py-1 rounded-full animate-pulse">
          Collision!
        </div>
      )}
      ...
      {speech && (
        <div className={`absolute -top-20 left-1/2 transform -translate-x-1/2 
          p-2 rounded-lg max-w-xs ${speech.type === 'think' ? 
          'bg-white italic before:content-[""] before:absolute before:bottom-0 before:left-1/4 before:w-3 before:h-3 before:bg-white before:rotate-45 before:rounded-sm' : 
          'bg-yellow-100 before:content-[""] before:absolute before:bottom-0 before:left-1/4 before:w-3 before:h-3 before:bg-yellow-100 before:rotate-45 before:rounded-sm'}
          border border-gray-300 text-sm shadow-md whitespace-normal break-words`}
      ...
      {/* Cat SVG with direction handling */}
